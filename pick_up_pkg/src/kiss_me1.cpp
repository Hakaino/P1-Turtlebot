...
float yaw_rotation(position orientation);
...
void start_kiss(){
  ...
  while(distance<(distance_first_back)){
  ...
  float yaw=yaw_rotation(current_position);
  ...
  while(distance<(distance_backwards)){
    ...
    driver(0.1,0);
    distance=sqrt(pow(turned_position.x_cor-current_position.x_cor,2)
                  +pow(turned_position.y_cor-current_position.y_cor,2));
    ros::spinOnce();
  }
}

//finding the yaw rotation in
float yaw_rotation(position orientation){
  tf::Quaternion quaternion(orientation.x_ori, orientation.y_ori,
                            orientation.z_ori, orientation.w_ori);
  ...
  void odomCallback(nav_msgs::Odometry odom_msg){
    ...
    current_position.y_ori=odom_msg.pose.pose.orientation.y;
    current_position.x_ori=odom_msg.pose.pose.orientation.x;
  }
